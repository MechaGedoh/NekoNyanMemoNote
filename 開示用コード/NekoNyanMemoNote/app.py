...
from .widgets import (
    MemoTextEdit, ReadOnlyFileSystemModel, CustomTreeView, CustomTabBar, AutoTextSettingsDialog
)
...
class MemoApp(QMainWindow):
    ...
    def load_file_content(self, editor):
        if not isinstance(editor, MemoTextEdit) or editor.is_loaded:
            return

        file_path = editor.file_path
        if file_path and os.path.isfile(file_path):
            content = self.fs_manager.load_memo_content(file_path)
    ...
    def toggle_read_only(self, file_path, read_only, model):
        ...
        if read_only and editor and editor.file_path == norm_path and not editor.isReadOnly():
            if editor.document().isModified():
                ...
                if self.fs_manager.save_memo_content(norm_path, content):
                    editor.document().setModified(False)
    ...
    def load_memo(self, file_path):
        ...
        if not os.path.isfile(norm_path):
            ...
            return
        self.ignore_save = True
        content = self.fs_manager.load_memo_content(norm_path)
        if content is not None:
    ...
    def save_current_memo(self, index=None):
        ...
        if not editor or self.ignore_save or not editor.file_path or editor.isReadOnly() or not editor.document().isModified(): return
        file_path = editor.file_path
        content = editor.toPlainText()
        if self.fs_manager.save_memo_content(file_path, content):
            editor.document().setModified(False)
    ...
    def add_folder_tab(self, name, path, add_plus_tab_after=True):
        ...
        memo_edit.textChanged.connect(self.update_footer_status)
        memo_edit.cursorPositionChanged.connect(self.update_footer_status)
