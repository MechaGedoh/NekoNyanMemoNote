...
def main():
    try:
        ...
        if hasattr(QApplication, 'AA_EnableHighDpiScaling'):
            QApplication.setAttribute(QApplication.Attribute.AA_EnableHighDpiScaling, True)
        if hasattr(QApplication, 'AA_UseHighDpiPixmaps'):
            QApplication.setAttribute(QApplication.Attribute.AA_UseHighDpiPixmaps, True)
        ...
        if QTNETWORK_AVAILABLE:
            ...
            if shared_memory.attach(QSharedMemory.AccessMode.ReadOnly):
                ...
                if socket.waitForConnected(500):
                    ...
                    QMessageBox.warning(None, "起動エラー", f"既に起動している {APP_NAME} に接続できませんでした。\n({socket.errorString()})\n\nプロセスが残っている可能性があります。")
                sys.exit(0)
            else:
                print("DEBUG: No existing instance found, creating new instance")
